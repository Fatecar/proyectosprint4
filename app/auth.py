...
def activate():
    try:
        ...
        if request.method == 'GET': 
            ...
            db = get_db()
            attempt = db.execute('select * from activationlink where challenge="'+number+'" and state="'+utils.U_UNCONFIRMED+'"').fetchone()

            if attempt is not None:
                db.execute(
                    'update activationlink set state="'+utils.U_CONFIRMED+'" where id="'+str(attempt['id'])+'"'
                )
                db.execute(
...
def confirm():
    try:
        ...
        if request.method == 'POST': 
            ...
            db = get_db()
            attempt = db.execute(
                'select * from forgotlink where challenge="'+authid+'" and state="'+utils.F_ACTIVE+'"'
            ).fetchone()
            
            if attempt is not None:
                db.execute(
                    'update forgotlink set state="'+utils.F_INACTIVE+'" where id="'+str(attempt['id'])+'"'
                )
                salt = hex(random.getrandbits(128))[2:]
                hashP = generate_password_hash(password + salt)   
                db.execute(
                    'update user set password="'+hashP+'", salt="'+salt+'" where id="'+str(attempt['userid'])+'"'
                )
                db.commit()
                return redirect(url_for('auth.login'))
            else:
...
def change():
    try:
        ...
        if request.method == 'GET': 
            ...
            db = get_db()
            attempt = db.execute(
                'select * from forgotlink where challenge="'+number+'" and state="'+utils.F_ACTIVE+'"'
            ).fetchone()
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if user is not None:
                ...
                db.execute(
                    'delete from forgotlink where state="'+utils.F_INACTIVE+'"and userid="'+str(+user['id'])+'"'
                )
                db.execute(
                    'insert into forgotlink (userid, challenge, state) values ("'+str(user['id'])+'", "'+number+'", "'+utils.F_ACTIVE+'")'
                )    
                db.commit()
